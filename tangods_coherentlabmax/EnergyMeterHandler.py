...
class EnergyMeterHandler(object):
    ...
    def create_serial_port(self):
        self._port = serial.Serial(
            port=self.port, baudrate=self.baud_rate, timeout=self.timeout, xonxoff=True
        )
    ...
    def set_value_energy_meter(self, set_command, value=""):
        ...
        """
        self.sendcmd(
            set_command.encode() + " ".encode() + value.encode() + "\n".encode()
        )
    ...
    def get_sensor_type(self):
        sens_type = self.clean_out(
            self.get_value_energy_meter("SYST:INF:PROB:TYPE?", 20)
        )[0]
        return sens_type
    ...
    def get_wavel_corr(self):
        if (
            self.clean_out(self.get_value_energy_meter("CONF:WAVE:CORR?", 30))[0]
            == "ON"
        ):
            return True
        else:
    ...
    def get_current_range(self, min=False):
        if min is True:
            return self.clean_out(
                self.get_value_energy_meter("CONF:RANG:SEL? MIN", 30)
            )[0]
