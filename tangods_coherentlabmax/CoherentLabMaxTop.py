...
from . import EnergyMeterHandler
...
class CoherentLabMaxTop(Device):
    ...
    unit_adj = attribute(
        label="Convert Units", dtype=unit_conv, access=AttrWriteType.READ_WRITE
    )
    adj_fact = attribute(
        label="Adjustment Factor", dtype="DevFloat", access=AttrWriteType.READ_WRITE
    )
    sensor_type = attribute(
        label="Sensor Typ", dtype="DevString", access=AttrWriteType.READ
    )
    pulse_period = attribute(
        label="Puls Periode", dtype="DevLong", unit="us", access=AttrWriteType.READ
    )
    ...
    current_range = attribute(
        label="Active Range", dtype=float, access=AttrWriteType.READ
    )
    auto_range = attribute(
        label="Auto Rangeing", dtype="DevBoolean", access=AttrWriteType.READ_WRITE
    )
    ...
    responsivity = attribute(
        label="Resposivity", dtype="DevString", access=AttrWriteType.READ
    )
    head_temp = attribute(
        label="Head Temperature", dtype="DevLong", unit="°C", access=AttrWriteType.READ
    )
    ...
    def init_device(self):
        ...
        try:
            self.pmSensor = EnergyMeterHandler.EnergyMeterHandler(
                self.Port, baud_rate=self.Baudrate
            )
        ...
        self.pmSensor.set_value_energy_meter(
            "CONF:STAT:BSIZ:PULS", "100"
        )  # sets batch size
        self._sampling_rate = 1
        self.pmSensor.set_value_energy_meter(
            "CONF:RANG:AUTO", "OFF"
        )  # turns off auto rangeing
        self.pmSensor.set_value_energy_meter(
            "CONF:STAT:RATE:PULS", "1"
        )  # sets sampling rate
        self.pmSensor.set_value_energy_meter(
            "CONF:DISP:STAT"
        )  # sets display mode to statistics
        self.pmSensor.set_value_energy_meter(
            "STAT:INIT"
        )  # initiates statistcs measurement
        self.pmSensor.set_value_energy_meter(
            "CONF:STAT:RMO", "AUT"
        )  # automatic restart of stat batch
        self.pmSensor.set_value_energy_meter(
            "CONF:STAT:STAR"
        )  # start statistics aquisition
        self.pmSensor.set_value_energy_meter(
            "CONF:READ:HEAD", "OFF"
        )  # turns of the data header
        self.pmSensor.set_value_energy_meter(
            "CONF:STAT:DISP", "MEAN,MIN,MAX,STDV"
        )  # sects the send statistcal data
        self.pmSensor.set_value_energy_meter(
            "CONF:READ:SEND", "PRI,UNIT,PER"
        )  # sects the send current data
        self.pmSensor.set_value_energy_meter(
            "CONF:READ:CONT", "LAST"
        )  # sets the buffer so only the last data entry is saved
    ...
    def allways_execute_hook(self):
        self.pmSensor.set_value_energy_meter(
            "STAT:INIT"
        )  # makes shure the stat is running
